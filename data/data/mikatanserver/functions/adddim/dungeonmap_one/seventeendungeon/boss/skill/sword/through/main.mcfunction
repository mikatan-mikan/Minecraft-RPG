...
summon armor_stand ^0.5 ^ ^ {Tags:["SBthroughSwordAr","SBthroughSwordArBe","SeventeenDunArmor"],Invisible:1b,Invulnerable:1b,HandItems:[{id:"minecraft:iron_sword",Count:1b}],ShowArms:1b,Pose:{RightArm:[0.0f,0.0f,0.0f]},DisabledSlots:4144959}

playsound entity.snowball.throw master @a[distance=..80] ~ ~ ~ 2 0 1

execute as @e[tag=SBthroughSwordArBe] at @s unless score @s throughSwordTimer matches 1..120 store result entity @s Rotation[0] float 1 run data get entity @e[tag=SeventeenDunBoss,limit=1] Rotation[0] 
...
function mikatanserver:adddim/dungeonmap_one/seventeendungeon/boss/skill/sword/through/move