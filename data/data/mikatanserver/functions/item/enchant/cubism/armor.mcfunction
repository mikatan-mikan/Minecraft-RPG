...
execute as @e[type=armor_stand,distance=..6,tag=cubismAr] as @e[distance=..3.5,type=#mikatanserver:cube] store result score @s MobNowHP run data get entity @s Health
execute store result score @s CubismLevel run data get entity @s SelectedItem.tag.cubism[]
execute as @e[type=armor_stand,distance=..6,tag=cubismAr] as @e[distance=..3.5,type=#mikatanserver:cube] run scoreboard players operation @s MobNowHP -= @p[tag=ExecuteCubism] CubismLevel
execute as @e[type=armor_stand,distance=..6,tag=cubismAr] as @e[distance=..3.5,type=#mikatanserver:cube] store result entity @s Health float 1 run scoreboard players get @s MobNowHP
execute as @e[type=armor_stand,distance=..6,tag=cubismAr] if entity @e[distance=..3.5,type=#mikatanserver:cube] run kill @s
