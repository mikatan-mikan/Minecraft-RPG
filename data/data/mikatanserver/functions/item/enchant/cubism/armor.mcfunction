...
execute as @e[type=armor_stand,distance=..6,tag=cubismAr] at @s if entity @e[distance=..1,type=#mikatanserver:cube] as @a[tag=ExecuteCubism,nbt={SelectedItem:{tag:{CustomModelData:155},id:"minecraft:carrot_on_a_stick"}}] at @s run function mikatanserver:item/enchant/cubism/magma_sword/main
